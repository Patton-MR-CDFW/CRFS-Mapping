...
# read in i1 for 2016 to 2024
new_oe <- fread(file = here('RCode', 'PR', 'Dat16toPresent', 'Data', 'i1_data_16to24.csv'), fill = TRUE) %>%
  mutate_all(as.character) %>%
  rename(DAYSF = daysf) 
...
write.csv(notused, 'Outputs/PR/NotUsed/i1/notused.csv', row.names = F, na = "")
...
write.csv(notused2, 'Outputs/PR/NotUsed/i1/notused2.csv', row.names = F, na = "")
...
byyear = notused4 %>% group_by(year) %>%
  count()
write.csv(notused4, 'Outputs/PR/NotUsed/i1/notused4.csv', row.names = F, na = "")
...
write.csv(oe_by_id_agg, "Outputs/PR/oe.csv", na = "", row.names = F)
write.csv(notused_summary, "Outputs/PR/oe_notused_summary.csv", na = "", row.names = F)
