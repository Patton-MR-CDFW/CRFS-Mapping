...
oc_by_id_agg = read.csv("Outputs/PR/oc.csv", na.strings = "")
rc_by_id_agg = read.csv("Outputs/PR/rc.csv", na.strings = "")
oe_by_id_agg = read.csv("Outputs/PR/oe.csv", na.strings = "")
...
all_by_id= all_by_id %>%
  ...
  select(id, year, Block, SP_CODE, Common_Name_catch, Common_Name_effort, TripType_Description_catch, TripType_Description_effort, Ob_Weighed_Fish, Ob_AvKWgt, Total_Obs_Fish_Caught, Rep_Released_Dead, Rep_Kept, Rep_Released_Alive, Total_Released_Fish_Caught, Total_Rep_Fish_Caught, Days, Cntrbs, Vessels, AnglerDays) 
...
not_in_shape = notused2 %>%
  group_by(Block) %>%
  count()
write.csv(notused2, 'Outputs/PR/NotUsed/notused2.csv' )
...
write.csv(obs_outliers, 'Outputs/PR/PotentialOutliers/ObsFish_Outliers.csv', na = "")
...
write.csv(rc_outliers, 'Outputs/PR/PotentialOutliers/RepFish_Outliers.csv', na = "")


write.csv(all_by_id, 'Outputs/PR/all_by_id.csv', row.names = F, na = "")
