...
all_locations = read.csv("Outputs/PR/location_cleaned.csv", na.strings = "") %>%
...
oc_16on <- fread(file = here('RCode', 'PR', 'Dat16toPresent', 'Data', 'i3_data_16to24.csv'), fill = TRUE) %>%
  mutate_all(as.character)
...
write.csv(notused, 'Outputs/PR/NotUsed/i3/notused.csv', row.names = F, na = "")
...
write.csv(notused2, 'Outputs/PR/NotUsed/i3/notused2.csv', row.names = F, na = "")
...
write.csv(notused3, 'Outputs/PR/NotUsed/i3/notused3.csv', row.names = F, na = "")
...
write.csv(weight_outliers, 'Outputs/PR/PotentialOutliers/OC_weight.csv', na = "")
...
write.csv(oc_by_id_agg, "Outputs/PR/oc.csv", na = "", row.names = F)
write.csv(notused_summary, "Outputs/PR/oc_notusedsummary.csv", na = "", row.names = F)
