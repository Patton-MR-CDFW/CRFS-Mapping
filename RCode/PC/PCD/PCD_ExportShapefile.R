...
working_directory = r"(C:\Users\MPatton\OneDrive - California Department of Fish and Wildlife\CRFS_Mapping\Outputs\PR\)"
...
output = read.csv(here('Outputs','PC','PCD', 'PCD_all_by_id.csv')) %>%
  rename(id = ID_CODE)

# bring in species grouping. This species grouping can be adjusted or new groups can be created in the lookup excel. 
...
WHERE = "C:\\GIS\\CRFSMapping\\Outputs\\PCD\\"
...
aggregate_catch = filtered_catch %>%
  ...
  summarise(TotalCatch = sum(Total_Fish_Caught, na.rm = T),
            Kept = round(sum(Total_Kept_Fish_Caught, na.rm = T), 2),
            Released = round(sum(Total_Released_Fish_Caught, na.rm=T), 2)) 
...
layer_long = blocks.SP %>%
  ...
  select(BlockBox = NM_INDEX, Catch = Common_Name_catch, Trip = TripType_Description_effort, Samples = n_samples_effort, everything() )

long_name = paste0('PCD_',SPECIES, '_',TRIPTYPE, '_', MINIMUM_SAMPLES ,'_SAMPLES_long.shp')
st_write(layer_long, paste(WHERE, long_name), delete_layer=T)

# plot of average CPUA by year
library(ggplot2)
ggplot(layer_long %>% st_drop_geometry(), aes(factor(year), CPUA_All)) +
  geom_bar(stat = 'summary', fun='mean') +
  xlab('Year') +
  ylab ('CPUA ALL') +
  ggtitle(paste0(SPECIES,' ', TRIPTYPE))
...
joined |>
  dplyr::summarise(n = dplyr::n(), .by = c(Block, Common_Name_catch, TripType_Description_effort, year)) |>
  dplyr::filter(n > 1L) 
...
wide_name = paste0('PCD_', SPECIES, '_',TRIPTYPE, '_', MINIMUM_SAMPLES ,'_SAMPLES_', METRIC, '.shp')
...
wide_group_name = paste0('PCD_', SPECIES, '_',TRIPTYPE, '_', MINIMUM_SAMPLES ,'_SAMPLES_', '_GROUPS', '.shp')
...
pal = colorNumeric('Reds', layer_wide_grouped[[METRIC]], na.color = 'transparent')
...
leaflet() %>%
  ...
  addPolygons(data=layer_wide_grouped,
              ...
              fillColor = ~pal(layer_wide_grouped[[METRIC]]),
              highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE),
              popup=paste0('Block:', layer_wide_grouped$BlockBox, '<br>', 
                           'CPUA: ', round(layer_wide_grouped[[METRIC]],2), '<br>',
...
# Monthly Output ----------------------------------------------------------
# note that this section is a draft
library(lubridate)
library(stringi)
