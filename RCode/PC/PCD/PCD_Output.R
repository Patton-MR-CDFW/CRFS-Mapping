# Standalone script to compile all existing PCD script outputs (all years observed catch, reported catch and effort) together to a cleaned up version containing all information by ID
...
source('RCode/PC/PCD/PCD_Locations.R')
source('RCode/PC/PCD/PCD_ObservedCatch.R')
source('RCode/PC/PCD/PCD_ReportedCatch.R')
source('RCode/PC/PCD/PCD_Effort.R')



# read in the individual outputs from the observed catch, reported catch and effort scripts.
oc_by_id_agg = read.csv("Outputs/PC/PCD/PCD_ObservedCatch.csv", na.strings = "")
rc_by_id_agg = read.csv("Outputs/PC/PCD/PCD_ReportedCatch.csv", na.strings = "")
oe_by_id_agg = read.csv("Outputs/PC/PCD/PCD_Effort.csv", na.strings = "")
...
all_by_id = oc_by_id_agg %>%
  full_join(rc_by_id_agg, by = c("ID_CODE", "Block", "Common_Name", 'year')) %>%
  left_join(Sp, by = c("Common_Name", "SP_CODE" = "PSMFC_Code")) %>%
  full_join(oe_by_id_agg, by = c("ID_CODE", "Block", 'year'), suffix = c("_catch", "_effort"))

all_by_id = all_by_id %>%
  mutate(district = substr(ID_CODE, 5, 5))
...
write.csv(notused, 'Outputs/PR/NotUsed/notused.csv', row.names = F, na = "")


# remove catch where there is no effort and clean up table to necessary fields
all_by_id= all_by_id %>%
  ungroup() %>%
  filter(!(!is.na(Common_Name_catch) & is.na(Common_Name_effort))) %>%
  select(ID_CODE, year, Block, SP_CODE, Common_Name_catch, Common_Name_effort, TripType_Description_catch, TripType_Description_effort, Ob_Weighed_Fish, Ob_AvKWgt, Total_Obs_Fish_Caught, Rep_Released_Dead, Rep_Kept, Rep_Released_Alive, Total_Released_Fish_Caught, Total_Rep_Fish_Caught, Days, Cntrbs, Vessels, AnglerDays, district)


# replaces NAs with 0s for the columns that will be aggregated
# calculates total fish caught and total kept fish caught
# if needed should add weight and length info here
all_by_id = all_by_id %>%
  ...
  mutate(Total_Kept_Fish_Caught = Total_Obs_Fish_Caught + Rep_Kept) 


# final check for duplicates
dups = all_by_id %>%
  group_by(ID_CODE, Block, Common_Name_catch, TripType_Description_effort) %>%
  count() %>%
  filter(n > 1)
...
not_in_shape = notused2 %>%
  group_by(Block) %>%
  count()
write.csv(notused2, 'Outputs/PC/PCD/NotUsed/notused2.csv' )
...
write.csv(obs_outliers, 'Outputs/PC/PCD/PotentialOutliers/ObsFish_Outliers.csv', na = "")
...
write.csv(rc_outliers, 'Outputs/PC/PCD/PotentialOutliers/RepFish_Outliers.csv', na = "")


write.csv(all_by_id, 'Outputs/PC/PCD/PCD_all_by_id.csv', row.names = F, na = "")
